...
void makePhoneCall(String phoneNumber) async {
  // Remove all non-numeric characters (e.g., spaces, dashes, etc.)
  String cleanedPhoneNumber = phoneNumber.replaceAll(RegExp(r'[^0-9]'), '');

  // Convert to num (can be int or double depending on the input)
  int? numericPhoneNumber;
  try {
    numericPhoneNumber = int.parse(cleanedPhoneNumber); // Parse to integer
  } catch (e) {
    print("Error parsing phone number to numeric: $e");
    ScaffoldMessenger.of(context).showSnackBar(
  ...
  // Create a valid phone URI using the numeric phone number
  final Uri phoneUri = Uri(scheme: 'tel', path: numericPhoneNumber.toString());

  try {
    // Launch the phone dialer
    if (await canLaunchUrl(phoneUri)) {
      await launchUrl(phoneUri, mode: LaunchMode.externalApplication); // Open the dial pad
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
